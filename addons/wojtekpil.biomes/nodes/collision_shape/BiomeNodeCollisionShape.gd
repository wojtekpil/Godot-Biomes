...
func _ready():
	set_slot(0, false, 0, Color(0, 0, 0), true, 6, Color(1, 1, 0))
...
func setup_dialogs(base_control):
	...
	_file_dialog.add_filter("*.shape; Shape files")
	_file_dialog.add_filter("*.tres; Shape resource files")
...
func generate_resource(output_slot: int):
	...
	resource.shape = load(_shapefile)
	return resource


func restore_custom_data(data := {}):
	if "shapefile" in data:
		_load_mesh(data['shapefile'])


func export_custom_data():
	return {'shapefile': _shapefile}
